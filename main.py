import sys
import requests
from multiprocessing.dummy import Pool
from colorama import Fore, init

init(autoreset=True)

fr = Fore.RED
fg = Fore.GREEN

print(f"{fr}\nBACKDOOR / SHELLS HUNTER\n")
print(f"{fr}\nBACKDOOR / SHELLS HUNTER\n")
print(f"{fr}\nTool by papa player or @temuworkerhere On TG\n")
print(f"{fr}\nTool by papa player or @temuworkerhere On TG\n")
print(f"{fr}\nTool by papa player or @temuworkerhere On TG\n")
print(f"{fr}\nTool by papa player or @temuworkerhere On TG\n")
print(f"{fr}\nTool by papa player or @temuworkerhere On TG\n")
print(f"{fr}\nTool by papa player or @temuworkerhere On TG\n")
print(f"{fr}\nTool by papa player or @temuworkerhere On TG\n")

requests.urllib3.disable_warnings()

try:
    target_sites = [i.strip() for i in open(sys.argv[1], mode='r').readlines()]
except IndexError:
    script_name = sys.argv[0].split("\\")[-1]  # Fix for Windows path
    exit(f"\n  [!] Usage: python {script_name} <sites.txt>")

Pathlist = '''
wp-admin/admin.php
wso.php
wordpress.php
webadmin.php
b374k.php
wp-login.php
shell.php
c99.php
r57.php
minishell.php
priv8.php
upload.php
backdoor.php
simpleshell.php
adminer.php
phpinfo.php
database.php
index.php
wp-config.php
site.php
server.php
test.php
injector.php
gate.php
uploader.php
hack.php
root.php
admin.php
ajax.php
system.php
update.php
eval.php
023.jsp
08小组内部交流专用.asp
1.png
12309.php
1427683968524.jpg
2.png
3.png
3fexe Shell.asp
4.png
404 infiltrate team.asp
404 Not Found.php
404.php
404super.php
404webshell.php
529.php
80sec内部专用过世界杀软休积最小功能超强超猛宇宙第一.asp
__init__.py
A robust backdoor script made by Daniel Berliner.php
about.txt
ACat-src.zip
ACat-附数据库驱动-jdk1.5.jar
ACat-附数据库驱动.jar
ACat.jar
ACat_jdk1.5.jar
accept_language.php
Adminer - Compact database management.php
Ajax_PHP Command Shell.php
AK-74 Security Team Web Shell Beta Version.php
AK-74 Security Team.php
alfa3.php
alfav3.0.1.php
andela.php
angel.php
Ani-Shell.php
Antak Webshell.aspx
Antichat Shell v1.3.php
Antichat Shell. Modified by Go0o$E.php
Antichat Shell.php
Antichat Socks5 Server v 1.0.php
Antichat_Shell_v1.3.php
applypatch-msg.sample
Aria cPanel cracker version 1.0 - Edited By KingDefacer.php
ASP Cmd Shell On IIS 5.1.asp
asp wget drag database.asp
ASP.NET Web BackDoor.aspx
Asp_Aspx_Php_V1.jpg
Asp_Aspx_Php_V2.jpg
Asp_Aspx_Php一句话合集.txt
ASP_Client.html
AspRootkit 1.0 by BloodSword.asp
aspSH.v1.asp
ASPX
ASPX one line Code Client by amxku.aspx
ASPX Shell.aspx
aspxshell.aspx
ASPXspy by NightRunner.aspx
ASPXspy.aspx
AspxSpy2014Final.aspx
ASPX小马 - 黑兵社团.aspx
ASPYDrvsInfo.asp
ASPYDrvsInfo.php
audit_null.c
audit_null.patch
ava Server Faces MiniWebCmdShell 0.2 by HeartLESS.php
AventGrup-Sincap 1.0.php
Aventis KlasVayv 1.0.asp
awen asp.net webshell.aspx
axis2 利用小工具cat.aar.zip
axis2 利用小工具cat.pdf
Axis2Shell-master.zip
Ayyildiz Tim  -AYT- Shell v 2.1 Biz.php
Ayyildiz Tim  -AYT- Shell v 2.1 Biz.txt
azrail 1.0 by C-W-M.php
aZRaiLPhp v1.0.php
aZRaiLPhp_v1.0.php
B374k Beta ShElL V1.php
b374k-mini-shell-php.php.php
b374k.php.php
Back Connect.php
Backdoor php v0.1 Coded By Charlichaplin.php
backdoor.c
backdoorfr.php
BackerHack JSP Manage-System 1.0.jsp
Backup script on server.php
backupsql.php
base64.jspx
base64.md
bdotw44shell.php
bdshell.php
bitwise.php
BLaSTER.php
bloodsecv4.php
Bnkqbakq.php
boffmax_v1.0_web_shell_by_the-c0de_team(1).php
browser.jsp
by Bagheera.jsp
by EJDER.asp
by.php
bypass safemodel.php
bypass-iisuser-p.asp
bypass-login.php
bypass_RCE_php.gif
c0derz shell [csh] v. 0.1.1 release.php
c999shell.php
c99_locus7s.php
c99_madnet.php
c99_webshell.php
C99madShell v. 2.0 madnet edition.php
c99ud.php
caidao-20141213.zip
caidao-20160622.zip
caidao.jsp
Carbylamine PHP Encoder.php
CasuS 1.5.php
cat.jar
cat.jsp
cat.jspx
cfExec.cfm
cfexec.cfm
cfmShell.cfm
cfSQL.cfm
cgi-python.py
Cgitelnet.pl
change.php
CHANGELOG
changelog
Changing CHMOD Permissions Exploit.php
client.php
client.py
Client.py
client1.php
client2.php
cls_Base.php
cmd-asp-5.1.asp
cmd-simple.php
cmd.asmx
cmd.asp
cmd.aspx
cmd.c
cmd.cfm
cmd.jpg
cmd.jsp
cmd.jspx
cmd.php
cmd.pl
cmd.sh
cmd.war
cmd_win32.jsp
cmdasp.asp
CmdAsp.asp
cmdasp.aspx
cmdjsp.jsp
CmdServlet.class
CmdServlet.java
cmdsql.aspx
cmfshell.cmf
Code by Bin.aspx
code.php
ColdFusion.chm
Command Execution (win32).jsp
Command Shell.php
Command.aspx
commit-msg.sample
compat
config
config.aar
config.h
config.inc.php
config.m4
configkillerionkros.php
Confusion to encrypt php webshell.php
console.py
control
Coppermine Photo Gallery = 1.4.3 remote cmmnds xctn.php
COPYING
copyright
cpanel.php
CREDITS
CrystalShell v.1.php
css.jsp
css1.jsp
Customize.asmx
customize.js
Customize.soap
cw.php
Cyber Shell.php
cybershell.php
CyberSpy5.Asp
d00r_py3.py
darkBC.py.txt
DAws.php
db_mysql.class.php
db_mysql_error.inc.php
dc.pl
dC3 Security Crew Shell PRiV.php
dC3_Security_Crew_Shell_PRiV.php
DDoS attack.php
Deface Keeper 0.2.php
demo.gif
description
dev_core.php
devilzShell.asp
devilzShell.aspx
devilzShell.cgi
devilzShell.jsp
devilzShell.php
devshell.asp
devshell.cfm
devshell.md
Dive Shell 1.0 - Emperor Hacking Team.php
Dive_Shell_1.0_Emperor_Hacking_Team.php
DJ团队.asp
dns.asp
dns.aspx
dns.php
download 下载文件.asp
DTool Pro.php
Dx.php
Edited By KingDefacer.php
EgY_SpIdEr ShElL V2.php
Elmali Seker.asp
ELMALISEKER Backd00r.asp
erne.php
ex0shell.php
exim.pl
exp.php
Expdoor.com ASP专用小马.asp
FaTaL Shell v1.0 - Edited By KingDefacer.php
fatal.php
File upload.asp
file.asp
file.aspx
file.php
filesystembrowser.aspx
fileupload.aspx
findsock.c
forever5pi.asp
format
from_the_wild1.php
fsmonitor-watchman.sample
ftpsearch.php
functions.c
functions.h
g00nshell-v1.3.php
Gamma Web Shell.cgi
Gamma Web Shell.php
gdog.py
get.php
GetShell.html
GFS web-shell ver 3.1.7 - PRiV8.php
gfs_sh.php
GFS_web-shell_ver_3.1.7_-_PRiV8.php
gif87a.jpg
gif89a.asp
gif89a.jpg
GO.cgi.pl
GPL
GRP WebShell 2.0 release build 2018 (C)2006,Great.php
h4ck_Door.asp
h4ntu shell [powered by tsoi].php
h4ntu_shell_[powered_by_tsoi].php
hahahaha小马.JSp
hec.aspx
hiddens shell v1.php
hideme.cpp
hkmjj.asp
hmass (priv8 mass defacor).pl
host.php
Hypn.ashx
I-47 v1.3.php
I.N.F HACKING CENTER.asp
ice.cfm
icmpsh-m.c
icmpsh-m.pl
icmpsh-s.c
icmpsh.exe
icmpsh_m.py
img.jpg
iMHaPFtp.php
inc.pl
include.ashx
includer.php
includer.txt
index
index.htm
index.php
inDEXER And ReaDer.asp
ipcheck.php
ironshell.php
JFIF.asp
JFIF.jpg
JFoler 1.0.jsp
jpg_payload.php
jshell ver 0.1.jsp
jshell ver 1.0.jsp
JSP Backdoor Reverse Shell.jsp
JSP Shell 岁月联盟专用版本.jsp
jsp-reverse.jsp
jsp.jpg
JspDo Code By Xiao.3.jsp
JspHelper Codz By - Leo.jsp
jspshell.jsp
JspSpy Codz By - Ninty.jsp
JspSpy Codz By - Ninty_1.jsp
JspSpy Private Codz By - Ninty.jsp
JspSpy Private Codz By - Ninty_encode.jsp
Jspspy web~shell V1.0 ※MADE by 孤水绕城 QQ540410588.jsp
JspSpy.jsp
jspspy.jspx
jspspy_k8.jsp
JspTqz.jsp
JspWebShell By 绝对零度.asp
jspx.jspx
jspy.jsp
Jsp反弹shell.txt
JSRat.ps1
ka0tic.pl
KA_uShell 0.1.6.php
KAdot Universal Shell v0.1.6.php
KAdot_Universal_Shell_v0.1.6.php
killnc.php
Knull Shell.php
KOA ASP类 WebShell扫描工具.asp
kolang-bypass.php
kral.php
lama's'hell v. 3.0.php
lamashell.php
laudanum.php
LICENSE
LICENSE.html
LICENSE.md
LICENSE.txt
license.zip
list.asp
list.cer
list.cfm
list.jsp
list.php
list.pl
list.sh
list.txt
listfile.php
ListServlet.class
ListServlet.java
litteryi.txt
litteryixx.ASP
livehack.py
liveterm.py
Liz0ziM Private Safe Mode Command Execuriton Bypass Exploit.php
llehs.py
load_shell.php
Loader'z WEB Shell v 0.1.0.2.php
Loaderz WEB Shell.php
logger.py
logger小马.jsp
login.jsp
lolipop.php
Lolipop.php - Edited By KingDefacer.php
lostDC shell.php
lostDC.php
LOTFREE PHP Backdoor v1.5.php
lurm_safemod_on.cgi.pl
Macker's Private PHPShell.php
make2.php
Makefile
makewar.sh
MANIFEST.MF
Matamu Mat.php
matamu.php
megabor.php
Micro_Webshell.php
mima_abcd.jpg
mini.php
mmm.php
mod_joomla_shell.zip
mof提权带回显带清楚命令版本.php
moon_1php.php
Moroccan Spamers Ma-EditioN By GhOsT.php
Moroccan_Spamers_Ma-EditioN_By_GhOsT.php
MSF_README
msf_webacoo_module.rb
mssql.asp
mssql.aspx
MSSQL控制程序.asp
mycode12.cfm
myshell.php
Mysql Database.jsp
Mysql interface v1.0.php
MYSQL Manager -Asp.net Silic Group Hacker Army专用版本.aspx
Mysql udf by M4ster.php
MySQL Web Interface Version 0.8.php
MySQL Web Shell.php
mysql.aspx
Mysql_interface_v1.0.php
mysql_tool.php
mysql脱库.php
navicat_tunnel.php
nc.exe
NCC Shell v1.0.0.php
NCC-Shell.php
NetworkFileManagerPHP.php
NGH.php
ngx_http_pwnginx.c
Non-alphanumeric.php
nShell v1.0.php
nshell.php
nsT View.php
NTDaddy  v1.9.asp
NTDaddy v1.9.php
ntdaddy.asp
ntunnel_mysql.php
obfuscated-phpshell.php
obfuscated-punknopass.php
one.jsp
one.war
one1.jsp
oo.jpg
openfire-test_plugin.zip
Oracle Database.jsp
oracle.jsp
oracle.txt
pack-6a5fbf3093c3c63aa794c842cf9da81dc956d2cd.idx
pack-6a5fbf3093c3c63aa794c842cf9da81dc956d2cd.pack
pack-a9e6fede5fa9947e97aa566caa3357ec1c1844b4.idx
pack-a9e6fede5fa9947e97aa566caa3357ec1c1844b4.pack
packed-refs
pas.php
Password Hasher for PHP Shell 2.1.php
paxmac.jspx
PcAnywhere提权 Bin版本.asp
Perl Web Shell by RST-GHC.pl
perl-reverse-shell.pl
perlcmd.cgi
PH Vayv.php
PH_Vayv.php
PHANTASMA.php
Php Backdoor v 1.0 by ^Jerem.php
php MySQL Database Backup Script.php
PHP Shell.php
PHP Web Shell by oTTo.php
PHP 搜索可读可写目录脚本.php
php-backdoor.php
php-findsock-shell.php
php-include-w-shell.php
php-reverse-shell.php
PHP-Shell-Detector-master.zip
phpinfo.php
pHpINJ.php
PHPJackal v1.9.php
phppng.png
PHPRemoteView.php
PhpShell 2.0.php
phpshell17.php
PhpSpy Ver 2006.php
phpwebbackup.php
pHp一句话扫描脚本程序.php
PHP小马 - ExpDoor.com.php
PHP整站打包程序-By DoDo.php
PHP检测文件夹权限.php
php版iisspy.php
php读取iis.php
PHVayv.php
Phyton Shell.py
ping.c
plugin-shell.php
pnbak.css
pnbak.js
post-update.sample
PostgreSQL数据库操纵.php
pps-v1.0.pl
pps-v3.0.pl
pps-v3.5.pl
pps-v4.0.pl
pre-applypatch.sample
pre-commit.sample
pre-merge-commit.sample
pre-push.sample
pre-rebase.sample
pre-receive.sample
Predator.php
prepare-commit-msg.sample
print.png
Private x0rg Web Hosting Bypass.php
proxy.asp
proxy.php
punk-nopass.php
punkholic.php
pwnginx
pwnginx-master.zip
pwnginx.c
pwnginx.h
pwnshell - an interactive jsp shell.jsp
pws.php
pyspy.py
qsd-php-backdoor.php
r00ts小组过防火墙马.asp
r00ts无FSO组建大马.asp
r57.biz Dq99Shell.php
r57.php
r57_iFX.php
r57_kartal.php
r57_Mohajer22.php
r57shell v.1.42 - Edited By KingDefacer.php
r57shell.php
r57shell127.php
radhat.asp
rcpexp.pl
README
README.md
readme.md
Readme.md
readme.MD
readme.txt
Readme.txt
RedHat Hacker.asp
RedHat Hacker.asp明文版.asp
reGeorg-master.zip
reGeorgSocksProxy.py
remot shell.pl
Remote Explorer.asp
requirements.txt
response_packet_from_icmpsh_slave_containing_output_of_command_whoami.png
reverse.jsp
reverseshell-poc.php
revsshclient.py
revsshserver.py
RHTOOLS 1.5 BETA(PVT) Edited By KingDefacer.asp
RHTOOLS 1.5 BETA(PVT).asp
robot.php
rootshell.php
Rootshell.v.1.0.php
ru24_post_sh.php
rules
run.sh
running_icmpsh_master_on_attacker_machine.png
running_icmpsh_slave_on_target.png
s72 Shell v1.0 Codinf by Cr@zy_King.php
s72 Shell v1.1 Coding.php
s72_Shell_v1.1_Coding.php
Safe mode breaker.php
Safe0ver Shell -Safe Mod Bypass By Evilc0der.php
safe0ver.php
Safe_Mode Bypass PHP 4.4.2 and PHP 5.1.2.php
Safe_Mode_Bypass_PHP_4.4.2_and_PHP_5.1.2.php
scanner.php
sctp_reverse.py.txt
Security House - Shell Center - Edited By KingDefacer.php
Serv-U本地权限提升工具.php
Server Variables.asp
server.php
server.py
Server.py
server1.php
server2.php
servlet-api-3.04.jar
settings.php
setup.py
Shell [ci] .Biz was here.php
Shell Commander.php
shell.ascx
shell.asp
shell.aspx
shell.cfm
shell.cfm.html
shell.jpg
shell.jsp
shell.php
shellcode_generate.py
sidedoor
sidedoor.default
sidedoor.dirs
sidedoor.docs
sidedoor.install
sidedoor.links
sidedoor.postinst
sidedoor.postrm
sidedoor.service
sidedoor.upstart
Silic Group Hacker Army - BlackBap.Org.php
Silic Group php Webshell v3.php
Silic Group.jsp
Silic Group_cgi.pl
Silic Group_readme.txt
SimAttacker - Version  1.0.0 - Edited By KingDefacer.php
SimAttacker - Vrsion 1.0.0 - priv8 4 My friend.php
simattacker.php
simple-backdoor.php
simple-shell.jsp
simple.jsp
simple_cmd.php
Simple_PHP_backdoor_by_DK.php
SimShell 1.0 - Simorgh Security MGZ.php
SimShell_1.0_-_Simorgh_Security_MGZ.php
Sincap 1.0.php
SJavaWebManageV1.4.jsp
Small Shell - Edited By KingDefacer.php
Small Web Shell by ZaCo.php
small.php
smevk.php
smtpd.py
sniffer.php
SnIpEr_SA Shell.php
socket_live8.py
socks5.h
soldierofallah.php
Sosyete Safe Mode Bypass Shell - Edited By KingDefacer.php
sosyete.php
Spider PHP Shell (SPS-3.0).php
spjspshell.jsp
SPS-3.0免杀.php
spygrup.php
SQL.aspx
ssh_client_config_example
SST Sheller.php
stdafx.h
STHx 渗透小组专用 ASP小马.asp
STNC WebShell v0.8.php
Stored Procedure Execute.aspx
stres.php
struct.py
sudoers
sure.php
sys32.php
t57shell.php
telnet.cgi.pl
telnet.pl
test.ear
test.war
test3693.war
test_rsa.key
tiny.php
TNTHK加密小马.asp
toby57解析加密一句话木马.php
TODO
tryag.php
tunnel.ashx
tunnel.aspx
tunnel.js
tunnel.jsp
tunnel.php
tunnel.tomcat.5.jsp
udf.dll 专用网马.php
udpio.py
UnKnown 高级Vip防删收费版.asp
up.asp
up.jsp
up.php
up.pl
up.sh
up_win32.jsp
update.sample
upfile_write.asp
upfile_write.rar
Uploader.php
Uploading.php
UpServlet.class
UpServlet.java
usage.txt
Utils.java
view.aspx
W3D Shell.php
Web Shell.asp
Web Sniffer.aspx
Web-shell (c)ShAnKaR.php
web.xml
webacoo.pl
WebAdmin 2.X Final.aspx
Webcommander by Cr4sh_aka_RKL v0.3.9 NGH edition.php
webllehs.py
WebShell.cgi.pl
WebShell.php
webshell.rb
WebShell系列(一)---XML.txt
WebSniff 1.0  Powered by  C.C.T.aspx
Welcome To AK Team.asp
wh_bindshell.py
Win MOF Shell.php
WinX Shell.php
wordpress backdoor.php
WordPress Shell.php
Worse Linux Shell.php
Worse Linux Shell.php.php
wp-conf.php
wp-conten1_pass_KoR345Ker78DSa.php
write_asp_file.ashx
write_aspx_file.ashx
wso.aspx
WSO2.7 404 Error Web Shell.php
wso2.8.5.php
www.zjjv.com.php
xl.cfm
xml.asp
xml.aspx
xml.php
xnonymoux_webshell_ver_1.0.php
xshock-0.1.tar.gz
xslt.asp
xslt.aspx
xslt.php
xssshell-xsstunnell.zip
xx.ashx
xx.php
xx.png
XXOO.jsp
xxooxx.aspx
xynu-Normal University.asp
z8VSmO1418105414843.jpg
zaco.php
zacosmall.php
ZehirIV.asp
zend_config.w32.h
zip.func.php
ZoRBaCK Connect.php
ZyklonShell.php
上传小马.asp
上传小马.jsp
上传马.aspx
上传马.php
不灭之魂.asp
不灭之魂2013改进版本.asp
专版aspx汗血宝马.aspx
中国木马资源网- WwW.7jyewu.Cn.php
中国木马资源网-WwW.MumaSec.TK.php
从注册表中读存在路径.aspx
仗剑孤行搜索可读可写目录脚本.php
传说中的hcker.asp
传说中的草泥马4.0.asp
使用方法.txt
使用说明.txt
修改属性.asp
内网扫描header.jsp
内网探测.jsp
冰锋刺客.aspx
凝聚科技专用AspX大马 Bysunue.aspx
合成图片马命令.txt
啊D小工具 - 目录读写检测 [ASP版].asp
国外牛逼大马.aspx
图片马.jpg
土司搞基asp大马.asp
在线exp专用免杀版.php
在线数据库管理工具 1.5.asp
密码：889.asp
小强asp木马.asp
小红帽.asp
小马.jsp
很好用的扫可读可写目录asp脚本xwdir.asp
新型JSP小马支持上传任意格式文件.jsp
旁注 - 网站小助手.asp
星外-华众-新网-虚拟主机提权专用Webshell Mumaasp.com发布.asp
星外-华众-新网-虚拟主机提权专用Webshell.asp
木马帮V1.1-火舌版.asp
极其隐蔽的pHp小马穿插在正常页面中.php
法克僵尸大马.asp
海阳顶端网ASP木马＠2006PLUS - By Marcos.asp
火狐NEW WebShell.asp
灭天远程管理.jsp
炽天使.asp
牛逼免杀提权隐藏大马.asp
目录扫描.asp
目录扫描读写马.asp
红狼ASP木马--Anfly免杀版.asp
老V.jsp
脱库工具.php
苦咖啡专用.jsp
草莓webshell.asp
菊花聊天室.php
虚拟主机提权专用Webshell去后门版.asp
虚拟机主机提权大马.asp
银河舰队大马_2014版.asp
银河舰队大马_2015专版asp大马.asp
零魂PHP一句话木马客户端.htm
'''
Pathlist = [path.strip() for path in Pathlist.strip().split("\n")]

class Scanner:
    def __init__(self):
        self.headers = {
            'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 '
                          '(KHTML, like Gecko) Chrome/91.0.4472.124 Safari/537.36'
        }

    def get_correct_url(self, site):
        """Ensures correct HTTP or HTTPS usage."""
        if not site.startswith(("http://", "https://")):
            site = "http://" + site  
        return site.rstrip("/") 

    def check_page(self, full_url):
        """Checks if a given URL exists and is accessible."""
        try:
            response = requests.get(full_url, headers=self.headers, verify=False, timeout=10)
            
            if response.status_code == 200:
                if any(keyword in response.text.lower() for keyword in ["login", "username", "password", "admin"]):
                    print(f"{fg}[+] Found: {full_url} (Admin Login Page ✅)")
                else:
                    print(f"{fg}[+] Found: {full_url} (200 OK, but not an admin page)")
                with open("valids.txt", "a") as valid_file:
                    valid_file.write(full_url + "\n") #youskidder
            elif response.status_code in [301, 302]:  
                print(f"{fg}[~] Redirected: {full_url} → {response.headers.get('Location', 'Unknown')}")
            else:
                print(f"{fr}[-] Not Found: {full_url} ({response.status_code})")
        except requests.exceptions.RequestException as e:
            print(f"{fr}[!] Error: Could not connect to {full_url} ({e})")

    def scan_site(self, site):
        """Scans a site for all paths in parallel."""
        site = self.get_correct_url(site) #crtopapa
        
        
        full_urls = [f"{site}/{path}" for path in Pathlist]

       
        with Pool(100) as pool:
            pool.map(self.check_page, full_urls)

scanner = Scanner()

with Pool(10) as site_pool:  # Adjust thread count for sites if needed
    site_pool.map(scanner.scan_site, target_sites)
